...
# 1. Импортираме (logic.py вече е конфигурирал AI при import)
from logic import get_recipes_for_user, get_kaufland_promotions, get_lidl_promotions
import database as db

# 2. Инициализиране на Flask и DB
app = Flask(__name__)
db_conn = db.create_connection()
db.create_table(db_conn)


# 3. ⚠️ ПРЕМАХНАТО: Вече не конфигурираме AI тук, logic.py го направи.


# 4. Дефиниране на Маршрути (Routes)
...
def daily_scrape_job():
    ...
    try:
        ...
        get_lidl_promotions(job_db_conn)  # (Тази функция сега просто принтира "спрян")
        job_db_conn.close()
